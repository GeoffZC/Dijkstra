...
public class AStar{
    ...
    Scanner fileScan;
    int distance = 0;
    int heuristicDistance = INFINITY;
    int heuristicEstimate = 0;
    int oldDistance = INFINITY;
  ...
  public void traverseGraph() {
	...
	//In A* currentNode = distance (already polled)
    currentNode = distance;//extract min
    ...
    System.out.println();
    //compare old distance at goal to new distance
    if(distances[goal] < oldDistance) {
        oldDistance = distances[goal];
    }
    else {
        break;
    }
    ...
    for(i = 0; i < unvisited.size(); i++) {
      ...
      heuristicEstimate = distances[unvisited.get(i)] + costs[unvisited.get(i)][goal];
      nodeQueue.add(heuristicEstimate);
      if(heuristicDistance > nodeQueue.peek()) { //if able to update heuristicDistance save Dijkstra distance for currentNode selection
            heuristicDistance = nodeQueue.peek();
            distance = distances[unvisited.get(i)];
      }
    }
